...
public class FiltersAndMapExercise {
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter( n -> n.compareTo(name)==0 ).collect(toList());
    ...
    public static List<Integer> filterNull(){
        ...
        evenNumbers = input.filter( i -> i != null ).collect(toList()); //Objects::nonNull
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = input.filter( p -> p.getName().compareTo(name)==0 && p.getAge().equals(age)  ).collect(toList());
    ...
    public static List<Integer> doubleNumbers() {
        ...
        doubleNumbers = input.map( i -> i = i*2 ).collect(toList());
    ...
    public static List<String> getOnlyNames(List<Person> personList){
        ...
        collect = input.map( p -> p.getName()).collect(toList());
    ...
    public static Optional<Person> findFirstAllMalePersons(List<Person> personList){
        ...
        personOptional = input.filter( p -> p.isMale()==true ).findFirst();
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        overAge = input.allMatch( p -> p.getAge() < age );
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        //min = input.map( p -> p.getAge()).reduce( Integer.MAX_VALUE, (acc, x) -> Integer.min(acc, x) );

        min = input.collect(Collectors.minBy(Comparator.comparingInt(Person::getAge))).get().getAge();
    ...
    public static double findAgeAverage(List<Person> personList){
        ...
        sum = input.map(p -> p.getAge()).reduce(0, (acc, x) -> acc += x );
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = input.filter( p -> p.getName().startsWith("J")).map(p -> p.getNationality())
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().filter( i -> Math.sqrt(i) == (int) Math.sqrt(i))
    ...
    public static  int sumSalary(List<Person> personList){
        int sum =0;
        //TODO #17
        sum = personList.stream().map( p -> p.getSalary()).reduce( 0, (acc, x) -> acc += x);
