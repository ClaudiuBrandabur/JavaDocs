...
public class Exercise1 {
    ...
    public List<String> iteratingOverKeys(){
        ...
        for (String s: countries.keySet()) {
            if (s.startsWith("R")==true) seekingCountries.add(s);
        }
    ...
    public List<String> iteratingOverEntries(){
        ...
        for (Map.Entry s: countries.entrySet()) {
            if (s.getKey().toString().contains("Q") || s.getKey().toString().contains("q"))
                seekingCountries.add(s.getKey().toString().toLowerCase());
        }
    ...
    public String iteratingOverValues(){
        ...
        for (String s: countries.values()) {
            if (seekingCapital.length() < s.length()) seekingCapital = s;
        }
